...
def column_missing(ctx: GateContext) -> Verdict:
    ...
    its columns are whatever the upstream module produced, so it is skipped here."""
    want = ctx.design.columns()
    derived = {i.name for i in ctx.project.inputs.items if i.derived_from is not None}
