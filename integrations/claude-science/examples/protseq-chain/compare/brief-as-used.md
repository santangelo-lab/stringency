# Operator brief: stringency project `toy-compare` on `protseq`
...
## Setup, once per session
    ...
    PROJ = "/data/lab/projects/2026-09_stringency-exit_jrrose5/compare/toy-compare"
    def sx(cmd, intent):
...
- pipeline `toy-engine`, profile `standard`, judgment harness `subagent`, executor `apptainer`
...
this message. The data file is the table the `toy-process` project delivered; my brief names its path. A `.stringency.json` sidecar sits beside it, so the input item carries `derived_from` copied from the sidecar. The sample manifest is `/data/lab/projects/2026-09_stringency-exit_jrrose5/data/samples.csv`.

Method `/home/jrrose5/mytools/stringency/stringency-toy-method@v0.1.2`, pipeline `toy-engine`, parent directory `/data/lab/projects/2026-09_stringency-exit_jrrose5/compare`, project directory
`/data/lab/projects/2026-09_stringency-exit_jrrose5/compare/toy-compare`; keep the defaults for profile, harness, and executor.

Save my brief verbatim as `/data/lab/projects/2026-09_stringency-exit_jrrose5/compare/brief.md` (the skill's step 1) and write the three files
under `/data/lab/projects/2026-09_stringency-exit_jrrose5/compare/declarations/`. Every value needs a source in the manifest, the brief, or the
...
`init` runs with `--drafted-by agent --brief /data/lab/projects/2026-09_stringency-exit_jrrose5/compare/brief.md` and ends with a confirm hold.
...
Run Phase 0 only, report the output, and stop.
