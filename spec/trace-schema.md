...
| `executions` | yes | `runner` (`engine`/`operator`), `ticket`, `env_name`, `env_digest`, `env_status` (`verified`/`as_reported`), `command`, `exit_code`, `duration_ms`, stdout/stderr paths, `evidence_paths_json`, `observed_params_json` |
