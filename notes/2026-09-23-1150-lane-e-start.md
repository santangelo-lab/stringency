...
1. Owner, in the app: run `hosts/protseq-session-tests.md` sections B and C (the two 3.4 tests),
   then D (the toy through `stringency-analyze-toy-compare`, after publishing it with the cell in
   `publish-skills.md`). Send the transcript and the `deliver/` path to the next Lane E session,
   which writes the Measurements row, the 3.4 answers in the README, and
   `skills/toy-engine.phrasings.results.md` in the toy method repo.
